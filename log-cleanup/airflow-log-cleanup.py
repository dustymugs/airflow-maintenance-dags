...
clean out the task logs to avoid those getting too big.

airflow trigger_dag --conf '{"maxLogAgeInDays":30}' maintenance.log.cleanup
...
DAG_ID = f'{DAG_PREFIX}.log.cleanup'
...
# If this is set to 30, the job will remove those files that are 30 days old or odler
DEFAULT_MAX_LOG_AGE_IN_DAYS = Variable.get("max_log_age_in_days", 30)

# Whether the job should delete the logs or not. Included if you want
# to temporarily avoid deleting the logs
ENABLE_DELETE = True

# The number of worker nodes you have in Airflow. Will attempt to run
# this process for however many workers there are so that
...
with DAG(
    ...
    cleanup_tasks = {
        'log_cleanup_{}'.format(log_cleanup_id): BashOperator(
            task_id='log_cleanup_{}'.format(log_cleanup_id),
